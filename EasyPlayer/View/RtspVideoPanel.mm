...
#import "RtspVideoPanel.h"
#import "AudioManager.h"
#import "Masonry.h"

#define kContentInset 1

@interface RtspVideoPanel() <RtspVideoViewDelegate> {
    RtspVideoView *_activeView;
    
    RtspVideoView *primaryView;
...
- (RtspVideoView *)nextAvailableContainer {
    ...
    for (int i = 0; i < [_resuedViews count]; i++) {
        RtspVideoView *videoView = [_resuedViews objectAtIndex:i];
...
- (void)setActiveView:(RtspVideoView *)activeView {
...
- (void)setLayout:(IRtspVideoLayout)layout currentURL:(NSString *)url URLs:(NSArray<URLModel *> *)urlModels {
    ...
    for (int i = 0; i < diff; i++) {
        RtspVideoView *videoView = [[RtspVideoView alloc] init];
    ...
    for (int i = 0; i < rowCount; i++) {
        RtspVideoView *leftView = nil;
        NSMutableArray *viewsOneRow = [[NSMutableArray alloc] init];
        
        for (int j = 0; j < columnCount; j++) {
            RtspVideoView *view = [_resuedViews objectAtIndex:(i * columnCount + j)];
    ...
    if (url) {
        ...
        RtspVideoView *view = [_resuedViews firstObject];
...
- (void)addCameraRes:(id)sender {
    ...
    RtspVideoView *view = (RtspVideoView *)button.superview;
...
- (void)videoViewBeginActive:(RtspVideoView *)view {
...
- (void)videoView:(RtspVideoView *)view response:(int)error {
    if (view == _activeView) {
        [self.delegate activeVideoViewRendStatusChanged:view];
    }
}

- (void)videoView:(RtspVideoView *)view connectionBreak:(int)error {
    if (view == _activeView) {
        [self.delegate activeVideoViewRendStatusChanged:_activeView];
    }
}

- (void)videoViewWillTryToConnect:(RtspVideoView *)view {
    if (view == _activeView) {
        [self.delegate activeVideoViewRendStatusChanged:_activeView];
    }
}

- (void)videoViewDidiUpdateStream:(RtspVideoView *)view {
    if (view == _activeView) {
