...
@interface RtspSplitScreenViewController ()<RtspVideoPanelDelegate> {
...
@property (nonatomic, strong) RtspVideoPanel *panel;
...
@implementation RtspSplitScreenViewController
...
- (void)viewDidLoad {
    ...
    self.panel = [[RtspVideoPanel alloc] initWithFrame:self.panelFrame];
...
- (void) crossScreenWithDuration:(NSTimeInterval)duration isLeftCrossScreen:(BOOL)isLeft {
    ...
    if (segment.selectedSegmentIndex == 0) {
        RtspVideoView *videoView = self.panel.resuedViews.firstObject;
...
- (void) normalScreenWithDuration:(NSTimeInterval)duration {
    ...
    for (RtspVideoView *v in self.panel.resuedViews) {
